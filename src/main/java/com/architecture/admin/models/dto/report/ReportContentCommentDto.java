...
@Builder
@ExcelFileName(filename = "lang.report.content.comment.title.list")
public class ReportContentCommentDto implements ExcelDto {

    /**
     * contents_comment_report
     **/
    @ExcelColumnName(headerName = "lang.report.content.comment.idx")
    private Long idx; // 작품 댓글 신고 내역 idx
    private Integer state; // 신고 상태값 (0:취소 / 1:정상)
    @ExcelColumnName(headerName = "lang.report.content.comment.state")
    ...
    @ExcelColumnName(headerName = "lang.report.content.comment.reporter.id")
    private String reporterId; // 신고 회원 아이디
    @ExcelColumnName(headerName = "lang.report.content.comment.reporter.nick")
    private String reporterNick; // 신고 회원 닉네임
    private Integer type; // 댓글 OR 대댓글 구분값
    @ExcelColumnName(headerName = "lang.report.content.comment.type")
    ...
    @ExcelColumnName(headerName = "lang.report.content.comment.status")
    private String statusText; // 댓글 상태 문자 변환
    private String statusBg; // 댓글 상태 bg 색상
    private Integer view; // 댓글 노출 상태값 (0:비노출 / 1:노출)
    @ExcelColumnName(headerName = "lang.report.content.comment.view")
    private String viewText; // 댓글 노출 상태 문자 변환
    private String viewBg; // 댓글 노출 상태 bg 색상
    @ExcelColumnName(headerName = "lang.report.content.comment.content")
    private String content; // 신고 댓글 내용
    @ExcelColumnName(headerName = "lang.report.content.comment.writer.id")
    private String writerId; // 작성 회원 아이디
    @ExcelColumnName(headerName = "lang.report.content.comment.writer.nick")
    private String writerNick; // 작성 회원 닉네임
    @ExcelColumnName(headerName = "lang.report.content.comment.regdate")
