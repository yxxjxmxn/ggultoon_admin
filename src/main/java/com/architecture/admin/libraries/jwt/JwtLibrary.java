...
public class JwtLibrary {
    private static String secretKey = Base64.getEncoder().encodeToString("petSnsAdmin".getBytes()); // static 해야함 들고있어야해서 여러개쓸때는 여러개만들어놓고, 인자값에서는 구분값가져와서 각자 다른 키 사용하도록
    private static String secretKey1 = Base64.getEncoder().encodeToString("petSnsAdmin1".getBytes()); // static 해야함 들고있어야해서 여러개쓸때는 여러개만들어놓고, 인자값에서는 구분값가져와서 각자 다른 키 사용하도록
    ...
    public Integer validateAccessToken(String secretKeyType, JwtDto accessTokenObj) {
        ...
        try {
            ...
            if (!claims.getBody().getExpiration().before(new Date())) {
                return 1;
            } else if (claims.getBody().getExpiration().before(new Date())) {// 유효한 기간이 만료일경우
                return 2;
            }
